...
import { GuideAdminEmailTemplate, GuideSenderEmailTemplate } from "@/components/email-templates/guide";
...
export async function POST(req: NextRequest) {
    try {
        ...
        const adminEmail = await resend.emails.send({
            ...
            subject: "מישהו חדש נרשם בטופס לקבלת המדריך",
            react: GuideAdminEmailTemplate(data),
        });
        ...
        const senderEmail = await resend.emails.send({
            ...
            to: [data.email],
            subject: "המדריך שלך מוכן - 5 אוטומציות שכל בעל עסק צריך",
            react: GuideSenderEmailTemplate({ firstName: data.firstName }),
        });
